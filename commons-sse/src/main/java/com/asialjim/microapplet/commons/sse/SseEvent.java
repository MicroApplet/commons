...
public class SseEvent<T> {
    ...
    public static SseEvent<TextPayload> text(String t, String tts) {
        return new SseEvent<>(SseEventType.TEXT, new TextPayload(t, tts));
    }

    public static SseEvent<DataPayload<Map<String, Object>>> data(Object s, List<Map<String, Object>> d) {
        return new SseEvent<>(SseEventType.DATA, new DataPayload<>(s, d));
    }

    public static SseEvent<DataPayload<Map<String, Object>>> data(Object s, List<Map<String, Object>> d, String sum) {
        return new SseEvent<>(SseEventType.DATA, new DataPayload<>(s, d, sum));
    }

    public static SseEvent<UiPayload> ui(UiPayload p) {
        return new SseEvent<>(SseEventType.UI, p);
    }

    public static SseEvent<ErrorPayload> error(String c, String m) {
        return new SseEvent<>(SseEventType.ERROR, new ErrorPayload(c, m));
    }

    public static SseEvent<RequireInputPayload<Object>> requireInput(String q, String h) {
        return new SseEvent<>(SseEventType.REQUIRE_INPUT, new RequireInputPayload<>(q, h));
    }
