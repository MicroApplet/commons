...
public abstract class JacksonUtil<M extends ObjectMapper, B extends MapperBuilder<M, B>> {
    ...
    public static final JavaType STRING_TYPE = TypeFactory.unsafeSimpleType(String.class);
    ...
    public MapperBuilder<M, B> builder() {
        return this.builder;
    }
    ...
    public final <T> T toBean(byte[] is, Class<T> type) {
    ...
    public JsonNode toTree(byte[] resBuffer) {
        return objectMapper().readTree(resBuffer);
    }


    public JsonNode toTree(String resBuffer) {
        return objectMapper().readTree(resBuffer);
    }
}