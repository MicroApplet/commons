...
package com.asialjim.microapplet.commons.cons.human;
...
public enum Nationality {
    ...
    public static Nationality codeOf(String code) {
        return Arrays.stream(values()).filter(item -> Strings.CI.equals(code, item.getCode())).findFirst().orElse(None);
    }

    public static Nationality cnNameOf(String nationality) {
        return Arrays.stream(values()).filter(item -> Strings.CI.equals(nationality, item.getCnName())).findFirst().orElse(None);
    }
}