...
package com.asialjim.microapplet.commons.standard.utils;
...
public class SessionTokenUtil {
    ...
    /**
     * 设置外部化配置的密钥（由 SessionTokenProperties 在 Spring 启动时调用）。
     * 优先级高于 System.getProperty 和默认值。
     */
    public static void setConfiguredSecret(String secret) {
        SessionTokenUtil.configuredSecret = secret;
    }
    ...
    private static String normalize(String key) {
        int length = StringUtils.length(key);
        if (length == 16)
            return key;
        else if (length > 16)
            return key.substring(0, 16);
        else
            return key + "0".repeat(Math.max(0, 16 - length));
    }
